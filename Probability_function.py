...
class Probability_calculator():
    ...
    def Check_for_guarenteed_win(self, ant1, ant2, whowon=1):
        self.foundwin *= whowon
        if ant1 in self.seen:
            return
        self.seen.append(ant1)
        if ant1.id == ant2.id:
            self.final = self.foundwin
            return self.final
        for i in range(len(ant1.fights)):
            self.Check_for_guarenteed_win(ant1.fights[i][0], ant2, ant1.fights[i][1])
        return self.final



# ants[0].fights.append([ants[0], 1])
# ants[0].fights.append([ants[3], 1])
# ants[2].fights.append([ants[2], -1])
# ants[1].fights.append([ants[4], -1])
# ants[6].fights.append([ants[5], -1])
# ants[4].fights.append([ants[2], -1])

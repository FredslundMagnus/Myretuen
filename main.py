...
if debuggerMode:
    ...
    env = Myretuen()
    controller = Controller(env=env, agent1=Opponent(NNAgent(calcprobs=True, minmax=True, montecarlo=False, TopNvalues=3, cutOffdepth=3).loadModel('NNAgent15000-IMP-sample-length10-hist10')), agent2=NNAgent(calcprobs=True, minmax=True, montecarlo=True).loadModel('NNAgent15000-IMP-sample-length10-hist10'))

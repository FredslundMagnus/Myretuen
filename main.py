...
if debuggerMode:
    ...
    env = Myretuen()
    controller = Controller(env=env, agent1=Opponent([CleverRandom(calcprobs=True),NNAgent(explore=True, doTrain=False, impala=False, calcprobs=True, minmax=False, lossf='MME', K=1000, dropout=0, alpha=None, discount=0.99, lambd=0.9, lr=0.0002, TopNvalues=3, cutOffdepth=3, ValueCutOff=25, ValueDiffCutOff=5, ProbabilityCutOff=0.01, historyLength=10, startAfterNgames=10, batchSize=200, sampleLenth=10, network=[50, 25, 10], analyse=False).loadModel('NNAgent2017ELO')]), agent2=NNAgent(explore=True, doTrain=False, impala=True, calcprobs=True, minmax=True, lossf='MME', K=1000, dropout=0, alpha=None, discount=0.99, lambd=0.9, lr=0.0002, TopNvalues=2, cutOffdepth=3, ValueCutOff=25, ValueDiffCutOff=5, ProbabilityCutOff=0.02, historyLength=10, startAfterNgames=10, batchSize=200, sampleLenth=10, network=[50, 25, 10], analyse=False).loadModel('NNAgent2017ELO'))
